...
export class FeeStrategyProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.logger.log(
      `Initialized FeeStrategyProvider with max fee ceiling: ${this.config.maxFeeCeiling} stroops`,
    );
  }
  ...
  public async calculateFee(): Promise<CalculatedFee> {
    ...
    if (
      this.feeCache &&
      now - this.feeCache.fetchedAt < this.config.feeCacheTtlMs
    ) {
      this.logger.debug(
    ...
    try {
      ...
      const source: CalculatedFee['source'] =
        calculatedFee >= this.config.maxFeeCeiling ? 'max_fee' : 'dynamic_fee';
      ...
      this.logger.debug(
        `Calculated dynamic fee: ${feeStr} stroops (p75: ${p75}, multiplier: ${this.config.feeMultiplier}, capped: ${source === 'max_fee'})`,
      );
  ...
  private getBaseFee(): CalculatedFee {
    const calculatedFee = Math.min(Number(BASE_FEE), this.config.maxFeeCeiling);
    return {
  ...
  updateConfig(partialConfig: Partial<FeeStrategyConfig>): void {
    ...
    this.logger.log(
      `Updated fee strategy config: ${JSON.stringify(this.config)}`,
    );
  }
